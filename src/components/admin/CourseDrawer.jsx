...
export function CourseDrawer({ course }) {
  ...
  // and focus restored to whatever triggered it on close — the same pattern as
  // ProductDrawer and the storefront cart drawer. This component is
  // mounted/unmounted by its parent (keyed on course id), so "open" is simply
  // "mounted": show on mount, and let the native "close" event (Escape or our
  // own close()) drive navigation back to the plain list.
  //
  // Focus is moved to the close button explicitly rather than via `autoFocus`:
  // on a client-side navigation into this route, React's autoFocus and
  // showModal()'s attribute-scanning fallback don't reliably compose, and
  // focus lands on the invisible backdrop click-catcher instead.
  useEffect(() => {
  ...
  return (
    <dialog
      ref={dialogRef}
      aria-labelledby="course-drawer-heading"
    ...
    >
      {/* Click-catcher for backdrop dismissal. tabIndex/aria-hidden keep it
          out of the tab order and the accessibility tree; without that it
          swallows the first Tab and the initial focus. */}
      <button
        type="button"
        tabIndex={-1}
        aria-hidden="true"
      ...
      <div className="relative w-full md:w-[520px] h-full">
        ...
        >
          <div className="px-stack-lg py-stack-md border-b border-dl-rule flex items-center justify-between sticky top-0 bg-dl-chalk z-10">
            <h2
              id="course-drawer-heading"
              className="font-dl-sans text-dl-body-lg font-semibold text-dl-ink"
            >
            ...
            <button
              ...
              onClick={close}
              aria-label="Close"
              className="text-dl-charcoal hover:text-dl-ink transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-dl-signal focus-visible:outline-offset-2"
            >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <label className="flex items-center gap-1 text-xs text-dl-charcoal shrink-0">
                        <input
                          ...
                          onChange={(e) =>
                            updateLesson(l.key, "is_preview", e.target.checked)
                          }
                        />
                        Free preview
                      </label>
